...
### Homebrew tap — `solcreek/homebrew-dew`

brew is the dominant install verb for Mac dev CLI tools and reaches a
discovery surface (brew search, Brewfile lists, "new Mac setup" guides)
that npm does not. Sequenced after v0.6 npm refactor so both channels
share one mental model: download a signed release asset, never touch
the binary on the user's machine.

Formula requirements (non-negotiable):

- **Binary download only** via `on_macos` + `on_arm` / `on_intel` +
  `on_linux` blocks pointing at the release asset URL
- **No `system "codesign"` step.** Developer ID + notarization staple
  are already inside the Mach-O; brew tarball extraction preserves
  the staple. Re-signing would strip it and break VM commands
- **No `go build` / `head` block.** Releases are the trust artifact;
  source builds skip notarization entirely
- **`livecheck`** from GitHub Releases tags
- **Generated by GoReleaser brew tap publisher** on tag push — zero
  human bump, formula stays in lockstep with releases

`dew update` becomes brew-aware: when the running binary lives under a
brew prefix, delegate to `brew upgrade dew` rather than in-place
replacement, so brew metadata stays consistent with the actual file
on disk.

The orphan `homebrew-dew/` working tree from v0.1.0 (build-from-source
+ ad-hoc resign) must be deleted and restarted from the GoReleaser
template. Publishing it as-is would strip Developer ID on install and
burn first-impression trust with the exact audience the tap is for.
