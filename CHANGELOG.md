...
- **`dew run --confine` expands `SystemCallFilter=` `@`-groups.** A known group
  such as `@system-service` is now expanded host-side into its member syscalls
  from a table mirrored from a pinned systemd release (`v257`), instead of being
  surfaced as unenforced. The table is generated by `go generate
  ./internal/confine/` (`gen_syscallgroups.go` parses systemd's
  `src/shared/seccomp-util.c`, resolves inter-group references, and emits the
  cross-arch union of each group's members); the guest agent's per-arch name
  drop recovers each architecture's exact subset, so no group table ships in the
  agent. An **unknown** `@`-group voids the whole directive and is surfaced as
  unenforced with the offending token(s) named — dropping one silently would
  flip a denylist the wrong way. Verified by in-VM boot tests: the
  `@system-service` allowlist (~390 expanded syscalls) runs a Go binary to
  completion, a denied known group and an unknown group both behave as specified.

### Fixed

- **`SystemCallFilter=` allow/deny polarity flips clear prior unknown-group
  state.** A form of only unknown `@`-groups (no explicit names) no longer voids
  a later, enforceable directive after a polarity flip.
